"""ACP wrapper for Degen Claw trade submission via OpenClaw ACP CLI.

All trade execution goes through ACP jobs to Degen Claw agent.
Uses the OpenClaw ACP CLI (npx tsx bin/acp.ts) instead of the Python SDK.
"""
...
DEGEN_CLAW_WALLET = "0xd478a8B40372db16cA8045F28C6FE07228F3781A"
...
class AcpTradeRequest(BaseModel):
    """Trade request sent to Degen Claw via ACP."""
...
class AcpTradeResponse(BaseModel):
    """Response from Degen Claw ACP job."""
...
class AcpCloseRequest(BaseModel):
    """Request to close a position via ACP."""
...
class DegenClawAcp:
    """Submits trade jobs to Degen Claw via the OpenClaw ACP CLI.
    ...
    def _run_cli(self, *args: str, timeout: int = 60) -> subprocess.CompletedProcess:
        """Run an openclaw-acp CLI command."""
    ...
    def submit_trade(self, request: AcpTradeRequest) -> AcpTradeResponse:
        """Submit a trade request to Degen Claw."""
        if request.size_usd < MIN_TRADE_SIZE_USD:
            return AcpTradeResponse(
                success=False,
                error=f"Size ${request.size_usd:.2f} below Degen Claw minimum ${MIN_TRADE_SIZE_USD}",
            )
    ...
    def submit_close(self, request: AcpCloseRequest) -> AcpTradeResponse:
        ...
        requirements = {"coin": request.coin, "action": "close"}
    ...
    def discover_provider(self) -> str:
        """Return the known Degen Claw wallet address."""
